...
public class Answer {
    public static void calk(int matrix[][], int n) {
        /**
         *
        ...
        for (int q = 1; q <= n; q++) {//вычисляем количество блоков умножения для вычисления определителя матрицы исходя из ее размеров
            stringQuantity *= q;
        }
         ...
         * ЗАПОЛНЕНИЕ МАССИВА
         */
        for (int q = 0; q < n; q++) {//заполняем массив значений адреса первым блоком
            adress.add(q, 0);
        }
        ...
        while (globalCheck) {//цикл заполнения массива адресов
            check = true;

            for (int q = 0; q < n; q++) {//цикл проверки массива значений адреса на наличие одинаковых в элементов внем
                ...
                adress.set(q, -1);
                if (adress.contains(spare)) {//если массив значений адреса все равно содержит в себе проверяемый элемент
             ...
             * КОПИРОВАНИЕ МАССИВА
             */
            if (check) {//если адрес прошел отбраковку
                for (int x = 0; x < n; x++) {//то он заносится в массив адресов
            ...
            for (int x = n - 1; x >= 0; x--) {//цикл проверки каждого элемента с конца на соответствие n-тичной системе счисления
                if (adress.get(x) > n - 1) {//если значение больше, чем n
                    adress.set(x, 0);//то оно приравнивается нулю
                    adress.set(x - 1, adress.get(x - 1) + 1);//предыдущий элемент увеличивается на единицу
                }
            }

            if (adress.get(0) == adress.get(1) && adress.get(0) == n - 1) {//если первые два элемента массива равны n, то рассчеты можно прекращать
                globalCheck = false;
            }
        ...
        for (int y = 0; y < stringQuantity; y++) {//перебор строк в массиве адресов
            //создаем переменную значения умножения
            int resultMultiplication = 1;

            for (int x = 0; x < n; x++) {//перебор позиций в строке
        ...
        for (int q = 0; q < multiplicationsAdresses.size(); q++) {
            if ((q - 1) % 6 == 0 || (q - 2) % 6 == 0 || (q - 5) % 6 == 0) {//каждая вторая, третья и шестая строки (при счете от первой) модулируются как отрицательные
        ...
        int spareOld = -1;
        for (int n2 = n; n2 > 3; n2--) {//счет ступеней вычисления
            int spare = 1;
            for (int step = 1; step < n2; step++) {//вычисляем число для формулы
                spare *= step;
            }
            //переключетель умножения на "-1"
            boolean checkNegative = false;
            for (int numberFromStringQuantity = 0; numberFromStringQuantity < stringQuantity; numberFromStringQuantity++) {//счет номеров элементов
                if (numberFromStringQuantity % spare == 0 && numberFromStringQuantity != 0) {//если номер строки делится на раннее расчитанное число то
                    if (checkNegative) {//если переключатель включен
